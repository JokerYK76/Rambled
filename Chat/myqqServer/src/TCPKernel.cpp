...
int TcpKernel::Open()
{
    ...
    if(  !m_sql->ConnectMysql("localhost","root","159357","myqq")  )
    {
...
void TcpKernel::RegisterRq(int clientfd,char* szbuf,int nlen)
{
    ...
    //没有用户 可以注册 -- 写表
        ...
        sprintf( sqlStr , "insert into t_userInfo( id , name , icon , feeling) values ( %d ,'%s' ,20 ,'');"
                 ,id,rq->m_szUser);
...
void TcpKernel::UploadRq(int clientfd, char *szbuf, int nlen)
{
    ...
    if( m_mapIDToUserInfo.find( rq->m_friendId) != m_mapIDToUserInfo.end() )
    {
        ...
        sprintf( info->filePath , "/home/birdbird/colin/tmp/%s", rq->m_szFileName );
        info->filePos = 0;

...
//    m_weChatDlg->show();
    ...
    m_SystemTrayIcon->setIcon(QIcon(":/images/1.jpg"));
    m_SystemTrayIcon->setToolTip(tr("鸟鸟"));
...
void CKernel::slot_dealFriendInfo(char *buf, int nlen)
{
    ...
    {//没找到
        ...
        connect( chat , SIGNAL( SIG_SendFile(int , QString) ) ,  //发送消息和传输文件
...
//处理聊天请求 我接收到了别人发送的聊天 我显示  在服务器就直接转发了 不在 服务器回复 rs
    ...
    if( m_mapFileMD5ToFileInfo.find(MD5)!= m_mapFileMD5ToFileInfo.end() )
    {
        ...
        if( info->pFile->open(QIODevice::ReadOnly ) )
        {
            while( 1 )
            {
                ...
                m_tcpClient->SendData( (char*)&rq , sizeof(rq) );  //发数据
...
void CKernel::slot_dealUploadRq(char *buf, int nlen)
{
    ...
    connect( item , SIGNAL(SIG_fileReject()) , this , SLOT(slot_fileReject() ) );  //摁键绑定
...
void CKernel::slot_sendVideoFrame(QImage &img)
{
    ...
    //将image图片转换成 缓冲区
    QByteArray ba;
    QBuffer qbuf(&ba); // QBuffer 与 QByteArray 字节数组联立联系  //QBuffer为Qt里面的缓冲区类

...
//文件信息请求
#define _DEF_FILE_LIST_RQ   (_DEF_PROTOCOL_BASE + 4 )
//文件信息
#define _DEF_FILE_INFO      (_DEF_PROTOCOL_BASE + 5 )
//文件信息请求结构体
struct STRU_FILE_LIST_RQ{
    STRU_FILE_LIST_RQ():type(_DEF_FILE_LIST_RQ), userid(0){
...
struct STRU_FILE_INFO{
    STRU_FILE_INFO():type(_DEF_FILE_INFO), userid(0), fileid(0), size(0){
    ...
    char uploadTime[_MAX_PATH_SIZE];
    int size;//2147483647 2GB
    char dir[_MAX_PATH_SIZE];
    char md5[_MAX_SIZE];
...
////////上传文件//////
//上传文件请求
#define _DEF_UPLOAD_FILE_RQ       (_DEF_PROTOCOL_BASE + 12 )
//上传文件内容回复
#define _DEF_UPLOAD_FILE_RS       (_DEF_PROTOCOL_BASE + 13 )
//上传文件请求
struct STRU_UPLOAD_FILE_RQ
{
    STRU_UPLOAD_FILE_RQ():type(_DEF_UPLOAD_FILE_RQ)
      ,userid(0),size(0){
        memset( fileName , 0, sizeof(fileName) );
        memset( dir , 0, sizeof(dir) );
        memset( md5 , 0, sizeof(md5) );
...
{
    STRU_UPLOAD_FILE_RS(): type(_DEF_UPLOAD_FILE_RS)
      , userid(0), fileid(0),result(1){
        memset( md5 , 0, sizeof(md5) );
    }
    PackType type;
...
////////新建文件夹//////
//新建文件夹请求
#define _DEF_ADD_FOLDER_RQ       (_DEF_PROTOCOL_BASE + 14 )
//新建文件夹回复
#define _DEF_ADD_FOLDER_RS       (_DEF_PROTOCOL_BASE + 15 )
//新建文件夹请求
struct STRU_ADD_FOLDER_RQ
{
    STRU_ADD_FOLDER_RQ():type(_DEF_ADD_FOLDER_RQ)
      ,userid(0),size(0){
        memset( fileName , 0, sizeof(fileName) );
        memset( dir , 0, sizeof(dir) );
        memset( md5 , 0, sizeof(md5) );
...
{
    STRU_ADD_FOLDER_RS(): type(_DEF_ADD_FOLDER_RS)
      , result(1){
    }
    PackType type;
    int result;
...
/////快传/////
//上传文件回复
#define _DEF_QUICK_UPLOAD_RS       (_DEF_PROTOCOL_BASE + 16 )

struct STRU_QUICK_UPLOAD_RS
{
    STRU_QUICK_UPLOAD_RS(): type(_DEF_QUICK_UPLOAD_RS)
      , userid(0), fileid(0),result(1){
        memset( md5 , 0, sizeof(md5) );
    }
    PackType type;
...
/////////下载文件夹///////
//下载文件夹请求
#define _DEF_FOLDER_DOWNLOAD_RQ       (_DEF_PROTOCOL_BASE + 25 )
struct STRU_FOLDER_DOWNLOAD_RQ{
    STRU_FOLDER_DOWNLOAD_RQ():type(_DEF_FOLDER_DOWNLOAD_RQ)
      ,fileid(0){
        memset( fileName , 0, sizeof(fileName) );
        memset( dir , 0, sizeof(dir) );
    }
...
//获取上传位置
#define _DEF_GET_UPLOAD_POS_RQ (_DEF_PROTOCOL_BASE + 26 )
#define _DEF_GET_UPLOAD_POS_RS (_DEF_PROTOCOL_BASE + 27 )

struct STRU_GET_UPLOAD_POS_RQ{
    STRU_GET_UPLOAD_POS_RQ(): type(_DEF_GET_UPLOAD_POS_RQ), userid(0), fileid(0){
        memset(dir, 0, sizeof(dir));
    }
    PackType type;
    int userid;
...
struct STRU_GET_UPLOAD_POS_RS{
    STRU_GET_UPLOAD_POS_RS(): type(_DEF_GET_UPLOAD_POS_RS), pos(0), fileid(0), result(1){}
    PackType type;
    int pos;
    int fileid;
    int result;
...
//请求
//告知 传输位置
#define _DEF_CONTINUE_DOWNLOAD_RQ (_DEF_PROTOCOL_BASE + 28 )
//服务端有map存放 只需要fileid，但是如果没有了，那么就需要唯一标识文件的信息
struct STRU_CONTINUE_DOWNLOAD_RQ{
    STRU_CONTINUE_DOWNLOAD_RQ(): type(_DEF_CONTINUE_DOWNLOAD_RQ), userid(0), fileid(0), pos(0){
        memset(dir, 0, sizeof(dir));
    }
    PackType type;
    int userid;
